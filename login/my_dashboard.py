...
from auth import require_perms, current_user, logout
...
def render():
    ...
    def get_weather_forecast():
        ...
        params = {
            ...
            "daily": "precipitation_probability_max", "timezone": "Asia/Seoul"
        }
        try:
    ...
    # 로그인 유저 표시(상단, 로그아웃 버튼 없음)
    u = current_user() or {}
    roles_txt = ", ".join(u.get("roles", [])) or "-"
    st.markdown(f"**👤 {u.get('username','-')}** | {roles_txt}")
    ...
    if st.session_state.view_mode == 'overview':
        ...
        with st.container(border=True):
            ...
            if not sensor_df_all.empty:
                ...
                cols[2].metric("전체 평균 온도", f"{avg_temp:.1f} °C")
                cols[3].metric("전체 평균 습도", f"{avg_humi:.1f} %")
            else:
            ...
            if not pig_log_df_filtered.empty:
                ...
                if len(warning_pigs) > 0:
                    with st.expander("'주의' 개체 목록 보기"):
                        ...
                        st.dataframe(
                            warning_pigs_with_reason[["pig_id", "temp_rectal", "breath_rate", "주의 원인"]],
        ...
        with tab2:
            if not equipment_df_filtered.empty:
                ...
                if energy_data_filtered_by_date.empty:
                    ...
                    fig_energy_period = px.bar(
                        period_usage, title="장비별 기간 내 사용량 (kWh)",
    ...
    # 오른쪽 하단 고정 로그아웃 버튼만 유지
    st.markdown(
        '<div style="position: fixed; right: 20px; bottom: 20px; z-index: 9999;">'
        '<a href="?logout=1" target="_self" '
        'style="text-decoration:none; background:#ef4444; color:white; padding:10px 16px; border-radius:8px; font-weight:600;">로그아웃</a>'
        "</div>",
