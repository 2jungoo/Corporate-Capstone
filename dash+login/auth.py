...
def users_summary():
    ...
    """)
    return [
        {"id": r[0], "username": r[1], "created_at": r[2], "roles": r[3] or ""}
        for r in (rows or [])
    ]
...
def admin_users():
    ...
    """)
    return [
        {"id": r[0], "username": r[1], "is_active": r[2], "roles": r[3] or ""}
        for r in (rows or [])
    ]
...
def delete_user(user_id: int):
    # user_roles 먼저 지우고 users 삭제
    _exec("DELETE FROM user_roles WHERE user_id = :uid", {"uid": user_id})
    _exec("DELETE FROM users WHERE id = :uid", {"uid": user_id})